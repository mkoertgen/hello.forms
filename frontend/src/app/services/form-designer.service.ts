...
export class FormDesignerService {
  // TODO: inject service generated by orval here
  ...
  updateState(updates: Partial<FormDesignerState>): void {
    ...
    if (updates.schema) {
      newState.schema.meta.updatedAt = new Date().toISOString();
      // Update ID if title changed
      if (
        updates.schema.meta.title &&
        updates.schema.meta.title !== currentState.schema.meta.title
      ) {
        newState.schema.meta.name = this.generateTitleSlug(updates.schema.meta.title);
      }
    }
  ...
  saveFormSchema(schema: FormSchema): Observable<FormSchema> {
    ...
    const titleSlug = this.generateTitleSlug(schema.meta.title);
    const isUpdate = !!schema.meta.name || this.isExistingForm(schema);
    ...
    if (isUpdate) {
      ...
      // Create new form
      schema.meta.name = titleSlug;
      if (!schema.meta.createdAt) {
        schema.meta.createdAt = new Date().toISOString();
      }
  ...
  deleteForm(id: string): Observable<Object> {
  ...
  private mapSqlTypeToFormFieldType(sqlType: SqlColumnType, maxLength?: number): FormFieldType {
    switch (sqlType) {
      case 'integer':
      case 'float':
        return 'number';
